(() => {
  ...
  function calendarAction(item) {
    const disabled = !window.AI_CALENDAR?.eligibleForReminder?.(item);
    return `<button class="activation-guide-action ${disabled ? 'is-disabled' : ''}" type="button" data-activation-calendar data-calendar-reminder="${e(item?.id || '')}" ${disabled ? 'disabled' : ''}><span>03</span><strong>锁定截止时间</strong><small>${disabled ? '当前截止时间不可生成提醒' : '下载日历文件，避免错过提交'}</small></button>`;
  }

  function initialGuideMarkup(item) {
    return `<section class="activation-guide" data-activation-guide-panel data-activation-mode="initial">
  ...
  function compactStartedMarkup(state) {
    return `<section class="activation-guide activation-guide-started" data-activation-guide-panel data-activation-mode="compact">
  ...
  function expandedStartedMarkup(item, state) {
    return `<section class="activation-guide activation-guide-started activation-guide-started-expanded" data-activation-guide-panel data-activation-mode="expanded">
      <div class="activation-guide-heading"><div><span>ACTIVATED</span><h2>你已经真正启动这场比赛</h2><p>第一项执行动作已经完成。保持当前节奏，接下来只推进一个最小步骤。</p></div><strong>已启动</strong></div>
      <div class="activation-guide-grid">
        ${officialAction(item)}
        <button class="activation-guide-action activation-guide-action-complete" type="button" data-activation-focus-task ${state.nextTask ? '' : 'disabled'}><span>02 · DONE</span><strong>${state.nextTask ? '继续下一项任务' : '全部任务已完成'}</strong><small>${state.nextTask ? '当前任务已划掉，点击定位下一项未完成动作' : '这场比赛的本地任务清单已经全部完成'}</small></button>
        ${calendarAction(item)}
      </div>
      <small class="activation-guide-privacy">任务内容和备注只保存在当前浏览器；统计只记录预定义行为和公开赛事 ID。</small>
    </section>`;
  ...
  function guideMarkup(item, state, mode = 'default') {
    if (state.completed === 0) return initialGuideMarkup(item);
    if (mode === 'expanded') return expandedStartedMarkup(item, state);
    return compactStartedMarkup(state);
  }
  ...
  function renderGuide() {
    ...
    const existing = page.querySelector('[data-activation-guide-panel]');
    const existingMode = existing?.dataset.activationMode || '';
    ...
    const preserveExpandedLayout = state.completed > 0 && (existingMode === 'initial' || existingMode === 'expanded');

    existing?.remove();
    layout.insertAdjacentHTML('beforebegin', guideMarkup(item, state, preserveExpandedLayout ? 'expanded' : 'default'));

    const panel = page.querySelector('[data-activation-guide-panel]');
    if (preserveExpandedLayout && panel && existingHeight > 0) {
      panel.style.height = `${existingHeight}px`;
