(() => {
  ...
  function regionOf(item) {
    if (item.region === 'CN' || item.region === 'INTL') return item.region;
    try {
  ...
  function recommendationScore(item) {
    ...
    score -= (item.riskFlags || []).length * 25;
    return score;
  }
  ...
  function gradeBadge(item) {
    ...
    return `<span class="grade-badge grade-${e(grade.toLowerCase())}">${e(grade)}${grade === 'U' ? ' · 待核验' : ''}</span>`;
  }
  function brand() {
  ...
  function competitionDetail(id) {
    ...
    return `<div class="detail-page"><div class="breadcrumbs"><a href="#/competitions">比赛库</a>${icon('chevron', 14)}<span>${e(item.title)}</span></div><section class="competition-detail-hero"><div><div class="detail-status-row">${statusBadge(item.status)}<span>${collectionLabel(item)}</span><span>${regionLabel(item)}</span>${gradeBadge(item)}</div><h1>${e(item.title)}</h1><p>${e(item.summary)}</p><div class="detail-actions">${sourceLink}<button class="secondary-button" data-favorite="${e(item.id)}">${icon('heart', 16, favorite ? 'fill="currentColor"' : '')}${favorite ? '已收藏' : '收藏比赛'}</button></div></div><div class="deadline-panel">${deadlinePanel}</div></section><section class="detail-layout"><div class="detail-main"><section class="detail-block"><h2>比赛概览</h2><p>${e(item.description || item.summary)}</p></section><section class="detail-block"><h2>适合谁参加</h2><p>${e(item.audience)}</p><div class="tag-row large-tags">${(item.tags || []).map((tag) => `<em>${e(tag)}</em>`).join('')}</div></section><section class="detail-block"><h2>评级说明</h2><p>${item.grade === 'U' ? '该赛事尚未完成赛事级深度审核，当前只确认了公开入口和基础信息，不代表高含金量推荐。' : '评级依据权威性、履历价值、成长价值、证据置信度和风险标记综合得出。'}</p></section></div><aside class="detail-sidebar"><h3>关键信息</h3>${infoRow('network','地区',regionLabel(item))}${infoRow('target','赛道',item.track)}${infoRow('users','参赛形式',item.format)}${infoRow('graduation','难度',item.difficulty)}${infoRow('play','举办形式',item.mode)}${infoRow('award','奖金/权益',item.prizeNote)}${infoRow('checkCircle','赛事等级',item.grade || 'U')}${infoRow('network','证据置信度',confidenceLabels[item.confidence] || '未知')}${infoRow('clock','最近核验',formatVerified(item.lastVerifiedAt || item.updatedAt))}<div class="official-note"><strong>${e(item.sourceName || '公开来源')}</strong><br>推荐排序规则公开，商业推广不影响排名。<br>报名与规则请以官方页面为准。</div></aside></section></div>`;
  }
