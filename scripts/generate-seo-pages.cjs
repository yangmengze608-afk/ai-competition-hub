...
const CURATED_IDS = [
  ...
  'gemini-xprize-2026',
  'drivendata-dat-parkinsons-2026',
  'backblaze-generative-media-2026',
  'devpost-call-e',
  'devpost-oneaquahealth-ieee-hackathon',
  'devpost-ieee-climatechain-hack',
  'devpost-gatewayhacks-2026',
];

const RISK_LABELS = {
  'paid-entry': '报名或参赛可能产生费用，付款前需核对退费和发票规则',
  'platform-specific-build': '作品需要使用主办方指定平台或技术栈',
  'team-details-lock-after-deadline': '团队信息可能在截止节点后锁定',
  'detail-rules-require-login': '部分详细规则需要登录官方平台后查看',
  'qualification-stage-closed': '当前阶段可能仅对已通过前序资格赛的队伍开放',
  'continuous-submission-required': '比赛可能要求持续提交或按阶段更新结果',
  'electronic-certificate-only': '证书形式可能仅为电子版',
  'recognition-varies-by-school': '不同学校对赛事级别和加分认定可能不同',
  'restricted-eligibility': '参赛身份、地区、年龄或学校范围存在限制',
  'timezone-risk': '截止时间涉及海外时区，需自行换算并预留提交时间',
  'ip-terms': '作品知识产权和授权条款需要重点核对',
...
function isoDate(value) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return '2026-07-29';
...
function entryLabel(item) {
  ...
  return labels[item.entryStatus] || '需前往官方页面确认';
}
...
function sourceLabel(item) {
  ...
  return labels[item.sourceLevel] || '公开来源';
}
...
function selectCompetitions(competitions) {
  const eligible = competitions.filter((item) => (
    item
    && item.id
    && item.title
  ...
  const fallback = eligible
    ...
    .sort((a, b) => score(b) - score(a) || String(a.title).localeCompare(String(b.title), 'zh-CN'));

  for (const item of fallback) {
    if (selected.length >= PAGE_LIMIT) break;
    selected.push(item);
    seen.add(item.id);
  }

  if (selected.length < PAGE_LIMIT) {
    throw new Error(`Only ${selected.length} reviewed current competitions are eligible; ${PAGE_LIMIT} are required.`);
  }

  return selected.slice(0, PAGE_LIMIT);
}

function riskItems(item) {
  const labels = (item.riskFlags || []).map((flag) => RISK_LABELS[flag]).filter(Boolean);
  if (labels.length) return labels;
  return ['报名之前仍需核对资格、时区、费用、提交材料和知识产权条款'];
}

function pageTitle(item) {
  return truncate(`${item.title}：报名时间、参赛资格与含金量｜AI 赛场`, 64);
}

function metaDescription(item) {
  return truncate(`${item.title}由${item.organizer || '赛事主办方'}举办。查看截止时间、参赛资格、费用、奖金、独立价值判断、风险提示和官方报名入口。`, 155);
}

function faqItems(item) {
  const deadline = formatDate(item.deadline);
  const timezone = item.deadlineTimezone || '以官方页面为准';
  return [
    {
      question: `${item.title}什么时候截止？`,
      answer: `本站最近核验到的截止日期为 ${deadline}，记录时区为 ${timezone}。主办方可能临时调整赛程，正式报名和提交前必须再次查看官方页面。`,
    },
    {
      question: `${item.title}谁可以参加？`,
      answer: cleanText(item.eligibility || item.audience || '参赛资格需要在官方规则中进一步确认。'),
    },
    {
    ...
    },
    {
      ...
      answer: '先打开官方规则，核对身份资格、报名与最终提交截止时间、团队限制、费用和提交物，再决定是否建立参赛计划。',
    },
  ];
}
...
function renderBreadcrumb(item) {
  return `<nav class="breadcrumb" aria-label="面包屑"><a href="/">AI 赛场</a><span>›</span><a href="/competitions/">比赛专题</a><span>›</span><span>${escapeHtml(item.title)}</span></nav>`;
}
...
function renderJsonLd(item, faq) {
  ...
  const article = {
    ...
    citation: (item.evidenceUrls || []).map(safeUrl).filter(Boolean),
  };
  const breadcrumb = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
  ...
  return `<script type="application/ld+json">${safeJson([article, breadcrumb, faqPage])}</script>`;
}
...
function renderCompetitionPage(item, selected) {
  ...
  const risks = riskItems(item);
  const evidence = [...new Set([item.sourceUrl, ...(item.evidenceUrls || [])].map(safeUrl).filter(Boolean))];
  const tags = [statusLabel(item), item.track, item.region === 'international' ? '国际赛事' : '国内赛事', `${item.grade || 'U'} 级`];
...
<body>
  ...
  <main>
    ...
    <section class="landing-hero">
      ...
      <p class="hero-summary">${escapeHtml(item.summary || item.description)}</p>
    ...
    <div class="content-layout">
      <div class="content-main">
        <section class="content-card answer-card">
          <span class="section-kicker">QUICK ANSWER</span>
          <h2>这是什么比赛，值得参加吗？</h2>
          <p>${escapeHtml(item.description || item.summary)}</p>
          <p>${escapeHtml(item.auditSummary || '该赛事已完成基础信息核验。报名之前仍需回到官方页面确认最终规则。')}</p>
          <div class="decision-note"><strong>AI 赛场判断：</strong>评级 ${escapeHtml(item.grade || 'U')}，证据置信度 ${escapeHtml(item.confidence || '待确认')}。评级用于比较赛事质量，不承诺获奖、升学或就业结果。</div>
        </section>

        <section class="content-card">
          <span class="section-kicker">ELIGIBILITY</span>
          <h2>谁适合参加？</h2>
          <p>${escapeHtml(item.eligibility || item.audience || '参赛对象和资格需在官方规则中确认。')}</p>
          <ul class="check-list">
            <li>对 ${escapeHtml(item.track || '相关赛道')} 有学习或作品积累需求的人</li>
            <li>能够在截止前完成真实提交物，而不只是收藏比赛的人</li>
            <li>愿意先核对资格、费用、技术限制与知识产权条款的人</li>
          </ul>
        </section>
        ...
        <section class="content-card">
          ...
          <p class="muted">截止日期是本站最近一次核验时的记录，不构成实时保证。建议至少提前 6 小时完成最终提交。</p>
        </section>

        <section class="content-card">
          <span class="section-kicker">FIRST ACTION</span>
          <h2>现在应该先做什么？</h2>
          <ol class="action-list">
            <li><strong>打开官方规则：</strong>确认报名和最终提交是否为两个不同节点。</li>
            <li><strong>核对资格：</strong>确认身份、地区、团队人数、费用和指定技术平台。</li>
            <li><strong>确定提交物：</strong>写清需要提交的代码、作品、文档、视频或演示链接。</li>
            <li><strong>建立参赛计划：</strong>进入 AI 赛场，把规则拆成可勾选的执行任务。</li>
          </ol>
          <a class="inline-cta" href="${appUrl}">为这场比赛建立参赛计划 →</a>
        </section>

        <section class="content-card">
          <span class="section-kicker">FAQ</span>
          <h2>常见问题</h2>
          <div class="faq-list">${faq.map((entry) => `<details><summary>${escapeHtml(entry.question)}</summary><p>${escapeHtml(entry.answer)}</p></details>`).join('')}</div>
        </section>

        <section class="content-card">
          <span class="section-kicker">EVIDENCE</span>
          <h2>核验依据与官方来源</h2>
          <p>以下链接用于支撑本站的赛事信息与独立判断。进入第三方网站后，适用对方的规则和隐私政策。</p>
          <ul class="source-list">${evidence.map((url, index) => `<li><a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer nofollow">${index === 0 ? '官方比赛入口' : `核验依据 ${index + 1}`} ↗</a></li>`).join('')}</ul>
        </section>
      </div>

      <aside class="decision-sidebar">
        <section class="sidebar-card">
          <span class="section-kicker">AI 赛场评级</span>
          <div class="grade-value">${escapeHtml(item.grade || 'U')}</div>
          ${scoreMeter('权威性', item.authorityScore)}
          ${scoreMeter('履历价值', item.resumeValue)}
          ${scoreMeter('成长价值', item.growthValue)}
        </section>
        <section class="sidebar-card">
          <h2>关键信息</h2>
          <dl>
            <div><dt>赛道</dt><dd>${escapeHtml(item.track || '待确认')}</dd></div>
            <div><dt>难度</dt><dd>${escapeHtml(item.difficulty || '待确认')}</dd></div>
            <div><dt>语言</dt><dd>${escapeHtml(item.language || '待确认')}</dd></div>
            <div><dt>状态</dt><dd>${escapeHtml(statusLabel(item))}</dd></div>
            <div><dt>来源</dt><dd>${escapeHtml(sourceLabel(item))}</dd></div>
          </dl>
        </section>
      </aside>
    </div>
    ...
    <section class="related-section">
      <span class="section-kicker">RELATED COMPETITIONS</span>
      <h2>还可以比较这些比赛</h2>
      <div class="related-grid">${related.map((candidate) => `<a href="/competitions/${encodeURIComponent(candidate.id)}/"><span>${escapeHtml(candidate.track || 'AI 比赛')}</span><strong>${escapeHtml(candidate.title)}</strong><small>${escapeHtml(candidate.grade || 'U')} 级 · ${escapeHtml(formatDate(candidate.deadline))}</small></a>`).join('')}</div>
    </section>
  </main>
...
function renderDirectory(selected, lastModified) {
  const cards = selected.map((item) => `<article><div><span>${escapeHtml(item.track || 'AI 比赛')}</span><span>${escapeHtml(item.grade || 'U')} 级</span></div><h2><a href="/competitions/${encodeURIComponent(item.id)}/">${escapeHtml(item.title)}</a></h2><p>${escapeHtml(item.summary || item.description)}</p><dl><div><dt>截止</dt><dd>${escapeHtml(formatDate(item.deadline))}</dd></div><div><dt>形式</dt><dd>${escapeHtml(item.format || '待确认')}</dd></div></dl><a class="card-link" href="/competitions/${encodeURIComponent(item.id)}/">查看报名时间、资格与含金量 →</a></article>`).join('');
...
<head>
  ...
  <meta property="og:description" content="首批 ${selected.length} 场经过赛事级审核的 AI 比赛专题页。" />
  <meta property="og:url" content="${canonical}" />
  <meta property="og:image" content="${DOMAIN}/assets/social-card.svg" />
  <link rel="stylesheet" href="/seo-landing-v1.css?v=1" />
...
<body>
  ...
  <main>
    ...
    <section class="directory-hero"><span class="section-kicker">SEARCH LANDING PAGES · BETA</span><h1>值得进一步核对的 AI 比赛</h1><p>首批只展示已经完成赛事级审核的比赛。每一页都提供截止时间、参赛资格、费用、独立价值判断、风险提示与官方入口，而不是只做一次外链跳转。</p><div class="verification-line"><span>${selected.length} 场审核赛事</span><span>最近更新 ${escapeHtml(formatDate(lastModified))}</span><span>规则以主办方公告为准</span></div></section>
...
function main() {
  ...
  const selected = selectCompetitions(competitions);
  ...
  console.log(`Generated ${selected.length} static competition pages in ${path.relative(ROOT, outDir) || '.'}.`);
  console.log(selected.map((item) => `- ${item.id}: ${item.title}`).join('\n'));
}
